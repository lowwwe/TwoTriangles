#include "Matrix3.h"
#include <cmath>

Matrix3::Matrix3()
{
	m11 = 0.0;
	m12 = 0.0;
	m13 = 0.0;
	m21 = 0.0;
	m22 = 0.0;
	m23 = 0.0;
	m31 = 0.0;
	m32 = 0.0;
	m33 = 0.0;
}

Matrix3::Matrix3(double t_a11, double t_a12, double t_a13, double t_a21, double t_a22, double t_a23, double t_a31, double t_a32, double t_a33) :

	m11{ t_a11 },
	m12{ t_a12 },
	m13{ t_a13 },
	m21{ t_a21 },
	m22{ t_a22 },
	m23{ t_a23 },
	m31{ t_a31 },
	m32{ t_a32 },
	m33{ t_a33 }
{
}

Matrix3 Matrix3::operator+(const Matrix3 t_other) const
{
	return 	Matrix3{ 
		m11 + t_other.m11, 
		m12 + t_other.m12, 
		m13 + t_other.m13,
		m21 + t_other.m21, 
		m22 + t_other.m22, 
		m23 + t_other.m23,
		m31 + t_other.m31, 
		m32 + t_other.m32, 
		m33 + t_other.m33 
	};
	
}

Matrix3 Matrix3::operator-(const Matrix3 t_other) const
{
	return 	Matrix3{
		m11 - t_other.m11,
		m12 - t_other.m12,
		m13 - t_other.m13,
		m21 - t_other.m21,
		m22 - t_other.m22,
		m23 - t_other.m23,
		m31 - t_other.m31,
		m32 - t_other.m32,
		m33 - t_other.m33
	};
}

Matrix3 Matrix3::operator*(const Matrix3 t_other) const
{
	return Matrix3{
		m11 * t_other.m11 + m12 * t_other.m21 + m13 * t_other.m31,
		m11 * t_other.m12 + m12 * t_other.m22 + m13 * t_other.m32,
		m11 * t_other.m13 + m12 * t_other.m23 + m13 * t_other.m33,

		m21 * t_other.m11 + m22 * t_other.m21 + m23 * t_other.m31,
		m21 * t_other.m12 + m22 * t_other.m22 + m23 * t_other.m32,
		m21 * t_other.m13 + m22 * t_other.m23 + m23 * t_other.m33,

		m31 * t_other.m11 + m32 * t_other.m21 + m33 * t_other.m31,
		m31 * t_other.m12 + m32 * t_other.m22 + m33 * t_other.m32,
		m31 * t_other.m13 + m32 * t_other.m23 + m33 * t_other.m33,
	};
}

MyVector3 Matrix3::operator*(const MyVector3 t_vector) const
{
	return MyVector3{
		t_vector.x * m11 + t_vector.y * m12 + t_vector.z * m13,
		t_vector.x * m21 + t_vector.y * m22 + t_vector.z * m23,
		t_vector.x * m31 + t_vector.y * m32 + t_vector.z * m33,
	};
}

Matrix3 Matrix3::operator*(const double t_scale) const
{
	return Matrix3{ 
		m11 * t_scale, 
		m12 * t_scale, 
		m13 * t_scale,
		m21 * t_scale, 
		m22 * t_scale, 
		m23 * t_scale,
		m31 * t_scale, 
		m32 * t_scale, 
		m33 * t_scale 
	};
}

bool Matrix3::operator==(const Matrix3 other) const
{
	return { 
		m11 == other.m11 && 
		m12 == other.m12 && 
		m13 == other.m13 && 
		m21 == other.m21 && 
		m22 == other.m22 && 
		m23 == other.m23 && 
		m31 == other.m31 && 
		m32 == other.m32 && 
		m33 == other.m33 
	};
}

bool Matrix3::operator!=(const Matrix3 other) const
{
	return { 
		m11 != other.m11 || 
		m12 != other.m12 || 
		m13 != other.m13 || 
		m21 != other.m21 || 
		m22 != other.m22 || 
		m23 != other.m23 || 
		m31 != other.m31 || 
		m32 != other.m32 || 
		m33 != other.m33 
	};
}

Matrix3 Matrix3::transpose() const
{
	return Matrix3{ m11, m21, m31, m12, m22, m32, m13, m23, m33 };
}

double Matrix3::determinant() const
{
	return { m11 * m22 * m33 - m11 * m32 * m23 + m21 * m32 * m13 - m31 * m22 * m13 + m31 * m12 * m23 - m21 * m12 * m33 };
}

Matrix3 Matrix3::inverse() const
{
	if (determinant() == 0)
		return Matrix3{};
	else
	{
		double scale = 1 / determinant();
		Matrix3 answer{};
		answer.m11 = scale * (m22 * m33 - m23 * m32);
		answer.m12 = scale * (m13 * m32 - m12 * m33);
		answer.m13 = scale * (m12 * m23 - m13 * m22);

		answer.m21 = scale * (m23 * m31 - m21 * m33);
		answer.m22 = scale * (m11 * m33 - m13 * m31);
		answer.m23 = scale * (m13 * m21 - m11 * m23);

		answer.m31 = scale * (m21 * m32 - m22 * m31);
		answer.m32 = scale * (m12 * m31 - m11 * m32);
		answer.m33 = scale * (m11 * m22 - m12 * m21);

		return answer;
	}
}

Matrix3 Matrix3::rotationAntiClockwiseZ(const double t_angleRadians)
{
	return Matrix3{
		std::cos(t_angleRadians), -std::sin(t_angleRadians), 0.0,
		std::sin(t_angleRadians), std::cos(t_angleRadians), 0.0,
		0.0, 0.0, 1.0 };
}

Matrix3 Matrix3::rotationAntiClockwiseY(const double t_angleRadians)
{
	return Matrix3{
		std::cos(t_angleRadians), 0.0, std::sin(t_angleRadians),
		0.0 ,1.0 ,0.0,
		-std::sin(t_angleRadians), 0.0, std::cos(t_angleRadians)
	};
}

Matrix3 Matrix3::rotationAntiClockwiseX(const double t_angleRadians)
{
	return Matrix3{
		1.0 , 0.0 , 0.0,
		0.0 ,std::cos(t_angleRadians) ,-std::sin(t_angleRadians),
		0.0 , std::sin(t_angleRadians), std::cos(t_angleRadians) };
}

Matrix3 Matrix3::translationXY(const MyVector3 t_displacement)
{
	return Matrix3{
		1.0 , 0.0 , t_displacement.x,
		0.0 , 1.0 , t_displacement.y,
		0.0 , 0.0 , 1.0 };
}

Matrix3 Matrix3::scale(const double t_scalingfactor)
{
	return Matrix3{
		t_scalingfactor, 0.0 , 0.0,
		0.0 ,t_scalingfactor, 0.0,
		0.0 , 0.0 ,t_scalingfactor
	};
}
